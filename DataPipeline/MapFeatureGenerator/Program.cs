        ...
        foreach (var (tileId, _) in mapData.Tiles)
        {
            ...
            foreach (var way in mapData.Ways)
            {
                ...
                foreach (var tag in way.Tags)
                {
                   ...
                   // Console.WriteLine(tag.Key);
                    featureData.PropertyKeys.keys.Add((PropertyKeys) KeyConverter.getIdFromKey(tag.Key));
                ...
                foreach (var nodeId in way.NodeIds)
                {
                    ...
                    {
                        if (!featureData.PropertyKeys.keys.Contains((PropertyKeys) KeyConverter.getIdFromKey(key)))
                        {
                            featureData.PropertyKeys.keys.Add((PropertyKeys) KeyConverter.getIdFromKey(key));
            ...
            foreach (var (nodeId, node) in mapData.Nodes.Where(n => !usedNodes.Contains(n.Key)))
            {
                ...
                for (var i = 0; i < node.Tags.Count; ++i)
                {
                    ...
                    featurePropKeys.Add((PropertyKeys) KeyConverter.getIdFromKey(tag.Key));
            ...
            foreach (var t in featureIds)
            {
                ...
                {
                    Byte b = (byte) featureData.PropertyKeys.keys[i];
                    ReadOnlySpan<char> v = featureData.PropertyValues.values[i];

                    fileWriter.Write(stringOffset); // StringEntry: Offset
                    fileWriter.Write(b.ToString().Length); // StringEntry: Length
                    stringOffset += b.ToString().Length;
